...
ExBH		=[0 0];
ExBL		=[0 0];
Esi		=[ones(KBound,1); 4*ones(KSteps-KBound,1)];
tStep		=[	1 10 20 30 40 50 100 120 140 160 180 ...
				200 210 220 230 240 260 280 300 320 340 360 380 400];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for T=1:NSteps
	Ex(2:end)	=Ex(2:end)+0.5./Esi(2:end).*(Hy(1:end-1)-Hy(2:end));
	pulse			=exp(-0.5*((T0-T)/spread)^2);
	Ex(KSource)	=Ex(KSource)+pulse;
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% Boundary Condition: ABC  absorbing boundary condition
   Ex(1)		=ExBL(1);
	ExBL(1)	=ExBL(2);
	ExBL(2)	=Ex(2);
